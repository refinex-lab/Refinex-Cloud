...
public class CaptchaService {
    ...
    public CaptchaGenerateResponse generate() {
        try {
            ...
            throw new BusinessException(
                    ModuleConstants.MODULE_SECURITY,
                    SecurityErrorCode.CAPTCHA_GENERATE_FAILED.getCode(),
                    SecurityErrorCode.CAPTCHA_GENERATE_FAILED.getMessage(),
                    e
            );
        }
    }
    ...
    public void verify(String uuid, String code) {
        ...
        if (!StringUtils.hasText(uuid) || !StringUtils.hasText(code)) {
            throw new BusinessException(
                    ModuleConstants.MODULE_SECURITY,
                    SecurityErrorCode.CAPTCHA_REQUIRED.getCode(),
                    SecurityErrorCode.CAPTCHA_REQUIRED.getMessage()
            );
        }
        ...
        if (!StringUtils.hasText(correctCode)) {
            ...
            throw new BusinessException(
                    ModuleConstants.MODULE_SECURITY,
                    SecurityErrorCode.CAPTCHA_EXPIRED.getCode(),
                    SecurityErrorCode.CAPTCHA_EXPIRED.getMessage()
            );
        }
        ...
        if (!correctCode.equals(inputCode)) {
            ...
            throw new BusinessException(
                    ModuleConstants.MODULE_SECURITY,
                    SecurityErrorCode.CAPTCHA_INVALID.getCode(),
                    SecurityErrorCode.CAPTCHA_INVALID.getMessage()
            );
        }
