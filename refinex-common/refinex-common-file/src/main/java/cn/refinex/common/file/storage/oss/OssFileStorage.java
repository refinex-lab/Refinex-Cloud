package cn.refinex.common.file.storage.oss;
 ...
 * 阿里云 OSS 文件存储实现
 * <p>
 * 基于 S3 协议兼容层实现，endpoint 配置为 OSS 的 S3 兼容端点。
...
public class OssFileStorage implements FileStorage {
    ...
    public String upload(InputStream inputStream, String fileName, String contentType, Map<String, String> metadata) {
        try {
            ...
            log.info("文件已上传到阿里云 OSS，bucket={}, key={}", bucketName, storageKey);
            
            // 5. 返回完整存储路径（格式：configId:bucketName:key）
            return configId + ":" + bucketName + ":" + storageKey;

        } catch (Exception e) {
            log.error("文件上传到阿里云 OSS 失败，fileName={}", fileName, e);
    ...
    public InputStream download(String storageKey) {
        try {
            ...
            log.info("文件已从阿里云 OSS 下载，bucket={}, key={}", bucketName, key);
            return inputStream;

        } catch (Exception e) {
            log.error("文件从阿里云 OSS 下载失败，storageKey={}", storageKey, e);
    ...
    public void delete(String storageKey) {
        try {
            ...
            log.info("文件已从阿里云 OSS 删除，bucket={}, key={}", bucketName, key);

        } catch (Exception e) {
            log.error("文件从阿里云 OSS 删除失败，storageKey={}", storageKey, e);
    ...
    public String generatePresignedUrl(String storageKey, Duration expiration) {
        try {
            ...
            log.info("阿里云 OSS 预签名 URL 已生成，bucket={}, key={}, expiration={}", bucketName, key, expiration);
            return url;

        } catch (Exception e) {
            log.error("阿里云 OSS 预签名 URL 生成失败，storageKey={}", storageKey, e);
    ...
    public String initiateMultipartUpload(String fileName, String contentType) {
        try {
            ...
            log.info("阿里云 OSS 分片上传已初始化，bucket={}, key={}, uploadId={}", bucketName, key, s3UploadId);
            return encodedContext;

        } catch (Exception e) {
            log.error("阿里云 OSS 分片上传初始化失败，fileName={}", fileName, e);
    ...
    public String uploadPart(String uploadId, int partNumber, InputStream inputStream) {
        try {
            ...
            log.info("阿里云 OSS 分片已上传，bucket={}, key={}, partNumber={}, eTag={}", context.getBucketName(), context.getKey(), partNumber, eTag);
            return eTag;

        } catch (Exception e) {
            log.error("阿里云 OSS 分片上传失败，uploadId={}, partNumber={}", uploadId, partNumber, e);
    ...
    public String completeMultipartUpload(String uploadId, List<PartETag> parts) {
        try {
            ...
            log.info("阿里云 OSS 分片上传已完成，bucket={}, key={}, parts={}", context.getBucketName(), context.getKey(), parts.size());

            // 6. 返回完整存储路径（格式：configId:bucketName:key）
            return context.getConfigId() + ":" + context.getBucketName() + ":" + context.getKey();

        } catch (Exception e) {
            log.error("阿里云 OSS 分片上传完成失败，uploadId={}", uploadId, e);
