...
public class ContentTagController {
    ...
    @PostMapping
    @LogOperation(operateDesc = "创建标签", operationType = OperateTypeEnum.CREATE)
    ...
    @PutMapping("/{id}")
    @LogOperation(operateDesc = "更新标签", operationType = OperateTypeEnum.UPDATE)
    ...
    @DeleteMapping("/{id}")
    @LogOperation(operateDesc = "删除标签", operationType = OperateTypeEnum.DELETE)
    ...
    @DeleteMapping("/batch")
    @LogOperation(operateDesc = "批量删除标签", operationType = OperateTypeEnum.DELETE)
    ...
    @GetMapping("/list")
    @Operation(summary = "获取我的标签列表", description = "获取当前用户的所有标签")
    public ApiResult<List<ContentTagResponseDTO>> listMyTags() {
        List<ContentTagResponseDTO> tags = contentTagService.listByUser(LoginHelper.getUserId());
        return ApiResult.success(tags);
    }

    @GetMapping("/system")
    @Operation(summary = "获取系统标签列表", description = "获取所有系统标签（所有人共享）")
    public ApiResult<List<ContentTagResponseDTO>> listSystemTags() {
        List<ContentTagResponseDTO> tags = contentTagService.listSystemTags();
        return ApiResult.success(tags);
    }

    @GetMapping("/page")
    @Operation(summary = "分页查询我的标签", description = "分页查询当前用户的标签")
    @Parameter(name = "tagName", description = "标签名称（可选）")
    @Parameter(name = "pageNum", description = "页码", required = true, example = "1")
    @Parameter(name = "pageSize", description = "每页数量", required = true, example = "10")
    public ApiResult<PageResult<ContentTagResponseDTO>> pageMyTags(
            @RequestParam(required = false) String tagName,
            @RequestParam(defaultValue = "1") int pageNum,
            @RequestParam(defaultValue = "10") int pageSize) {
        PageResult<ContentTagResponseDTO> result = contentTagService.pageByUser(
                LoginHelper.getUserId(), tagName, new PageRequest(pageNum, pageSize));
        return ApiResult.success(result);
    }
}
