package cn.refinex.platform.service.dict;
...
import cn.refinex.platform.constants.DictionaryCacheConstants;
...
public class SysDictService {
    ...
    public Long createDictType(String dictCode, String dictName, String dictDesc, String remark, Integer status, Long operatorId) {
        ...
        if (dictTypeRepository.existsByCode(dictCode, null)) {
            throw new BusinessException(PLATFORM, DICT_TYPE_CODE_EXIST.getCode(), DICT_TYPE_CODE_EXIST.getMessage());
        }
    ...
    public boolean updateDictType(Long id, String dictName, String dictDesc, String remark, Integer status, Long operatorId) {
        ...
        if (exist == null) {
            throw new BusinessException(PLATFORM, DICT_TYPE_NOT_FOUND.getCode(), DICT_TYPE_NOT_FOUND.getMessage());
        }
    ...
    public boolean deleteDictType(Long id, Long operatorId) {
        ...
        if (CollectionUtils.isNotEmpty(dataList)) {
            throw new BusinessException(PLATFORM, DICT_TYPE_IN_USE.getCode(), DICT_TYPE_IN_USE.getMessage());
        }
    ...
    public SysDictType getDictTypeByCode(String dictCode) {
        // 先从缓存中读取
        String cacheKey = DictionaryCacheConstants.buildDictTypeCacheKey(dictCode);
        try {
    ...
    public Long createDictData(Long dictTypeId, String dictLabel, String dictValue, Integer dictSort, String cssClass, String listClass, Integer isDefault, String remark, Integer status, Long operatorId) {
        ...
        if (type.getStatus() != null && type.getStatus() == 1) {
            throw new BusinessException(PLATFORM, DICT_TYPE_DISABLED.getCode(), DICT_TYPE_DISABLED.getMessage());
        }
        // 校验字典标签、值的唯一性
        if (dictDataRepository.existsLabelInType(dictTypeId, dictLabel, null)) {
            throw new BusinessException(PLATFORM, DICT_DATA_LABEL_EXIST.getCode(), DICT_DATA_LABEL_EXIST.getMessage());
        }
        if (dictDataRepository.existsValueInType(dictTypeId, dictValue, null)) {
            throw new BusinessException(PLATFORM, DICT_DATA_VALUE_EXIST.getCode(), DICT_DATA_VALUE_EXIST.getMessage());
        }
    ...
    public boolean updateDictData(Long id, Long dictTypeId, String dictLabel, String dictValue, Integer dictSort, String cssClass, String listClass, Integer isDefault, String remark, Integer status, Long operatorId) {
        ...
        if (exist == null) {
            throw new BusinessException(PLATFORM, DICT_DATA_NOT_FOUND.getCode(), DICT_DATA_NOT_FOUND.getMessage());
        }
        // 校验字典类型是否存在
        SysDictType type = dictTypeRepository.selectById(dictTypeId);
        if (type == null) {
    ...
    public List<SysDictData> listDictDataByTypeCode(String dictCode) {
        ...
        // 先从缓存中读取
        String cacheKey = DictionaryCacheConstants.buildDictDataListCacheKey(dictCode);
        try {
            String cacheJson = redisService.string().get(cacheKey, String.class);
            if (StringUtils.isNotBlank(cacheJson)) {
    ...
    private void writeTypeCache(SysDictType type) {
        ...
        try {
            String key = DictionaryCacheConstants.buildDictTypeCacheKey(type.getDictCode());
            String json = JsonUtils.toJson(type);
            redisService.string().set(key, json, Duration.ofSeconds(DictionaryCacheConstants.DEFAULT_CACHE_TTL));
        } catch (Exception e) {
    ...
    private void writeDataListCache(String dictCode, List<SysDictData> list) {
        try {
            String key = DictionaryCacheConstants.buildDictDataListCacheKey(dictCode);
    ...
    private void invalidateCachesByDictCode(String dictCode) {
        try {
            String typeKey = DictionaryCacheConstants.buildDictTypeCacheKey(dictCode);
            String dataKey = DictionaryCacheConstants.buildDictDataListCacheKey(dictCode);
