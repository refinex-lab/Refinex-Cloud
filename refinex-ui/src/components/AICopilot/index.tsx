...
const AICopilot: React.FC<AICopilotProps> = ({ open, onClose }) => {
  ...
  const [sessionList, setSessionList] = React.useState<Conversation[]>(MOCK_SESSION_LIST);
  ...
  const [agent] = useXAgent<BubbleDataType>({
    ...
    model: 'deepseek-chat',
    dangerouslyApiKey: 'Bearer sk-5555ecxxxxxxx',
  });
  ...
  const chatHeader = (
    <div className={styles.chatHeader}>
      <div className={styles.headerTitle}>
        ✨ {intl.formatMessage({ id: 'component.aiCopilot.title' })}
      </div>
      <Space size={0}>
        <Button
          type="text"
    ...
    <div className={styles.chatList}>
      {messages?.length ? (
        ...
        <>
          <Welcome
            variant="borderless"
            title={intl.formatMessage({ id: 'component.aiCopilot.welcome.title' })}
            description={intl.formatMessage({ id: 'component.aiCopilot.welcome.description' })}
            className={styles.chatWelcome}
          />

          <Prompts
            vertical
            title={intl.formatMessage({ id: 'component.aiCopilot.prompts.title' })}
